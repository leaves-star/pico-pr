...
/* China NTC Thermistor */
/*
#define WTEMP_ALERT_BLUE    (2134)
#define WTEMP_ALERT_GREEN   (1188)
#define WTEMP_ALERT_YELLOW  (958)
*/
/* Suzuki genuine NTC Thermistor */
#define WTEMP_ALERT_BLUE    (1611)
#define WTEMP_ALERT_GREEN   (773)
#define WTEMP_ALERT_YELLOW  (600)
 ...
 * 
 * Due to the characteristics of translucent panels, white is easy to see,even in the daytime.
 * Colours other than white are hard to see during the day.
 * The three primary colours of R/G/B are slightly easier to see,
 * but composite colours, such as yellow, are particularly hard to see.
 * Therefore, the intensity of white is changed within the range of 50-75,
 * but single(R/G/B) colours are illuminated at 1.5 times the intensity,
 * and composite colours are illuminated at twice the intensity.
 * Since the panel normally emits almost white light, it is also advantageous in terms of power consumption.
 *
...
void Pixels_control(PIO pio, uint sm, uint32_t lux, uint16_t rpm, uint16_t wtemp,uint16_t air_temp) {
    ...
    int ii;
    uint8_t power;  
    constexpr int SINGLE_LED = 1.5;
    constexpr int COMP_LED = 2;
    ...
    if(1 == gpio_get(FLASHER_PIN)) {
        for(ii=0; ii < NUM_CIRCLE_PIXELS; ii++) {
            pixels[ii] = urgbp_u32(COL_ORANGE, power*COMP_LED);
        }
    } else */
     {
        if(rpm >= RPM_ALERT_RED) { // rpm over 9,500
            for(ii=0; ii < NUM_CIRCLE_PIXELS; ii++) {
                pixels[ii] = urgbp_u32(COL_RED, power*SINGLE_LED);
            }
            pixels[TACHO_NEEDLE_PIXEL]      = urgbp_u32(COL_RED,power*SINGLE_LED);
            pixels[TACHO_NEEDLE_PIXEL+1]    = urgbp_u32(COL_RED,power*SINGLE_LED);
        } else if(rpm >= RPM_ALERT_YELLOW) { 
            for(ii=0; ii < NUM_CIRCLE_PIXELS; ii++) {
                pixels[ii] = urgbp_u32(COL_YELLOW,power*COMP_LED);
            }
        } else {
            for(ii=0; ii < NUM_CIRCLE_PIXELS; ii++) {
    ...
    if(wtemp>WTEMP_ALERT_BLUE) { // Blue: Water temp under 49c
        pixels[WTEMP_NEEDLE_PIXEL]      = urgbp_u32(COL_BLUE,power*SINGLE_LED);
        pixels[WTEMP_NEEDLE_PIXEL+1]    = urgbp_u32(COL_BLUE,power*SINGLE_LED);
    } else if(wtemp>WTEMP_ALERT_GREEN) { // Green 50c to 79c
        pixels[WTEMP_NEEDLE_PIXEL]      = urgbp_u32(COL_GREEN,power*SINGLE_LED);
        pixels[WTEMP_NEEDLE_PIXEL+1]    = urgbp_u32(COL_GREEN,power*SINGLE_LED);
    } else if(wtemp>WTEMP_ALERT_YELLOW) {  // Yellow 80c to 89c
        pixels[WTEMP_NEEDLE_PIXEL]      = urgbp_u32(COL_YELLOW,power*COMP_LED);
        pixels[WTEMP_NEEDLE_PIXEL+1]    = urgbp_u32(COL_YELLOW,power*COMP_LED);
    } else {                // Red over 90c
        pixels[TACHO_NEEDLE_PIXEL]      = urgbp_u32(COL_RED,power*SINGLE_LED);
        pixels[TACHO_NEEDLE_PIXEL+1]    = urgbp_u32(COL_RED,power*SINGLE_LED);
        pixels[WTEMP_NEEDLE_PIXEL]      = urgbp_u32(COL_RED,power*SINGLE_LED);
        pixels[WTEMP_NEEDLE_PIXEL+1]    = urgbp_u32(COL_RED,power*SINGLE_LED);
    }
...
uint16_t adc_to_wtemp(uint16_t adc) {
    /*
    static uint16_t wtemp_adc_list[] = { // Water temperature list from 20c to 120c (china sensor)
    ...
    */
    ...
    static uint16_t wtemp_adc_list[] = { // Water temperature list from 20c to 120c (Suzuki genuine sensor)
        2826,2786,2746,2705,2664,2623,2582,2540,2498,2456,2414,2372,2330,2288,2246,2204,2163,2121,2080,2039,1998,1958,1917,1878,1838,
        1799,1760,1722,1685,1647,1611,1574,1539,1504,1469,1435,1402,1369,1336,1305,1274,1243,1213,1184,1155,1127,1099,1072,1046,1020,
         995, 970, 946, 923, 900, 877, 856, 834, 813, 793, 773, 754, 735, 716, 698, 681, 664, 647, 631, 615, 600, 585, 571, 556, 543,
         529, 516, 503, 491, 479, 467, 456, 445, 434, 423, 413, 403, 393, 384, 375, 366, 357, 348, 340, 332, 324, 317, 309, 302, 295, 288,
    };
